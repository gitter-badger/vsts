...
lifetime.applicationsList()
    ...
    let newApp = new ltclt.Application();
...
lifetime.deploymentsGetStatus(depKey)
    .then((res) => {
    let deploylog = res.body.DeploymentLog;
    let deployStatus = res.body.DeploymentStatus;
    let fullLog = deploylog.map((_log) => {
        return _log.Message;
    }).join("\r\n");
})
    .catch((err) => {
